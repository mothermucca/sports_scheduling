# based on sports_scheduling_sat.cc, Copyright 2010-2018 Google LLC
#
...
class VarArraySolutionPrinter(cp_model.CpSolverSolutionCallback):
    ...
    def __init__(self, fixtures, getter, csv_name):
        cp_model.CpSolverSolutionCallback.__init__(self)
        self.__fixtures = fixtures
        self.__getter = getter
        self.__solution_count = 0
        self.__csvfile = None  # gets assigned in the next call
        self.__writer = self.get_csv_writer(csv_name)
    ...
    def get_csv_writer(self, csv_name):
        self.__csvfile = open(csv_name, 'w', newline='')
...
def get_scheduled_fixtures(solver, fixtures, pools):
    pool_membership = {home: home_pool
                       for (home_pool, pool) in enumerate(pools)
                       for home in pool}
...
def check_file_collision(csv_basename):
    ...
    idx = 1
    check_name = csv_basename
    match = re.search(r"\.csv", check_name)
    if not match:
        print('looking for a .csv ending in passed in CSV file name.  Did not find it, so appending .csv to',
              csv_basename)
        csv_basename += ".csv"

    check_name = csv_basename
    while os.path.exists(check_name):
        check_name = re.sub(r"\.csv", "_{}.csv".format(idx), csv_basename)
        idx += 1
        # or just get rid of it, but that is often undesirable
        # os.unlink(csv_basename)
    return check_name


def csv_dump_results(scheduled_games, csv_basename):
    check_name = check_file_collision(csv_basename)
    with open(check_name, 'w', newline='') as csvfile:
        fieldnames = ['day', 'home', 'away', 'home pool', 'away pool']
...
def pool_pool_accumulator(pool_vs_pool, row):
    pool_vs_pool[row['home pool'] - 1][row['away pool'] - 1] += 1
    return pool_vs_pool


def team_pool_accumulator(team_vs_pool, row):
...
def screen_dump_pool_checks(scheduled_games, pools):
    ...
    pool_vs_pool = reduce(pool_pool_accumulator, scheduled_games, pool_vs_pool)

    # this loop accumulates match counts for team vs pool
    team_vs_pool = reduce(team_pool_accumulator, scheduled_games, team_vs_pool)
...
def collect_pool_play_fixtures(teams, pools, match_days, fixtures):
    pool_play = [
        [fixture_slice(fixtures, match_days, [t], pool) + fixture_slice(fixtures, match_days, pool, [t])
         for pool in pools]
        for t in teams
    ]
    return pool_play
...
def collect_pool_balance_fixtures(pools, match_days, fixtures):
    pool_balance = [
        [fixture_slice(fixtures, match_days, pooli, poolj)
         for poolj in pools]
        for pooli in pools
    ]
    return pool_balance
...
def fixture_slice(fixture, days, home_games, away_games):
    return [fixture[day][home][a] for day in days for home in home_games for a in away_games]
...
def add_one_game_per_day(match_days, matches_per_day, teams, fixtures, model):
    ...
    #  for d in match_days for t in teams]
...
def add_one_matchup_per_round_robin(teams, fixtures, model, matchups, matchups_exact, unique_games, matches_per_day,
                                    num_match_days):
    ...
    #
    # represents all possible pairings of fixtures over the round-robin
    # period between could probably between team t and team o.
    ...
    last_days = [d for d in range((matchups - 1) * days_to_play, num_match_days)]
    if matchups_exact:
...
def add_max_home_stand_constraint(teams, at_home, model, num_match_days, max_home_stand):
    # forbid sequence of 3 homes or 3 away games in a row
    for t in teams:
...
def create_breaks(model, teams, num_match_days):
    ...
    breaks = [
        ...
        for (break_index, (t, d)) in enumerate([(t, d)
                                                for t in teams
                                                for d in range(num_match_days - 1)])]
    return breaks


def breaks_constraint(breaks, teams, at_home, num_match_days, model, listall=False):
    # side effects through list comprehension, yet again
    # noinspection PyStatementEffect
    [
        [model.AddBoolOr([at_home[d][t], at_home[d + 1][t], breaks[break_index]]),
         model.AddBoolOr([at_home[d][t].Not(), at_home[d + 1][t].Not(), breaks[break_index]]),

         model.AddBoolOr([at_home[d][t].Not(), at_home[d + 1][t], breaks[break_index].Not()]),
         model.AddBoolOr([at_home[d][t], at_home[d + 1][t].Not(), breaks[break_index].Not()])]
        for (break_index, (t, d)) in enumerate([(t, d)
                                                for t in teams
                                                for d in range(num_match_days - 1)])]
    ...
    #
    # except they are a little more efficient, I believe.  Wrote it up on a blog post

    # literature aside, I'm finding in practice that is num_match_days
    ...
    if listall:
        ...
        if num_match_days % 2:
            model.Add(sum(breaks) <= num_match_days + 1)
        else:
            model.Add(sum(breaks) <= num_match_days)
    else:
...
def model_matches(num_teams,
    ...
    match_days = range(num_match_days)
    ...
    total_games = num_match_days * num_matches_per_day
    ...
    fixtures = daily_fixtures(model, num_teams, num_match_days)  # all possible games
    at_home = daily_at_home(model, num_teams, num_match_days)  # all possible games
    ...
    pool_play = collect_pool_play_fixtures(teams, pools, match_days, fixtures)
    pool_balance = collect_pool_balance_fixtures(pools, match_days, fixtures)
    ...
    add_one_game_per_day(match_days, num_matches_per_day, teams, fixtures, model)
    ...
    # assert 0
    add_one_matchup_per_round_robin(teams, fixtures, model, matchups, matchups_exact, unique_games, num_matches_per_day,
                                    num_match_days)

    add_max_home_stand_constraint(teams, at_home, model, num_match_days, max_home_stand)

    breaks = create_breaks(model, teams, num_match_days)

    breaks_constraint(breaks, teams, at_home, num_match_days, model, listall)
...
def report_results(solver, status, fixtures, pools, num_teams, num_match_days, time_limit=None, csv_name=None):
    ...
    screen_dump_pool_checks(scheduled_games, pools)
    ...
    if status != cp_model.OPTIMAL and solver.WallTime() >= time_limit:
        ...
        print(
            'A better solution than %i might be found by adding more time using the --timelimit command line option' % solver.ObjectiveValue())

    if csv_name:
        csv_dump_results(scheduled_games, csv_name)
...
def main():
    ...
    parser.add_argument('-d,--days', type=int, dest='num_match_days', required=True,
    ...
    # set default for num_match_days
